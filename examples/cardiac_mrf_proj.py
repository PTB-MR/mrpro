...
#%%
pname = Path('/echo/_allgemein/projects/pulseq/measurements/2024-10-16_T1MES_cMRF/meas_MID00043_FID06421_20240716_cMRF_1D_705rep_trig_delay800ms/')
...
if t1_ref is not None and t2_ref is not None:
    m0_ref = [1,] * len(t1_ref)
    (signal_ref,) = epg_mrf_fisp.forward(torch.as_tensor(m0_ref), 
                                         torch.as_tensor(t1_ref), 
                                         torch.as_tensor(t2_ref))
...
for ind in range(len(t2_match)):
    ax[ind].set_title(f'T2: {int(t2_match[ind])} | T1: {int(t1_match[ind])} (GT: T2: {int(t2_ref[ind])} | T1: {int(t1_ref[ind])})')
