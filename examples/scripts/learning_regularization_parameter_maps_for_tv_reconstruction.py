...
# %% tags=["hide-cell"] mystnb={"code_prompt_show": "Show download and dataset details"}
import tempfile
...
# %% [markdown]
# Below, we define some auxiliary functions that we will need for retrospectively simulating undersampled
# and noisy k-space data


# %% tags=["hide-cell"] mystnb={"code_prompt_show": "Show data normalization details"}
...
# %% tags=["hide-cell"] mystnb={"code_prompt_show": "Show data vistualization details"}
...
# %% [markdown]
# We now define the unrolled PDHG network by instantiating the `ParameterMapNetwork2D` as well as the
# `AdaptiveTVNetwork2D` classes.
...
# %% [markdown]
# We now apply the pre-trained network to the scanner data. Note that we need to adapt
# the trajectory and the reconstruction matrix to the scanner data, as the scanner data is acquired at a lower
# resolution than the target resolution we want to reconstruct. We therefore need to adapt the
# trajectory and the reconstruction matrix accordingly. The super-resolution factor is set to 2.0
# to match the factor used for the training data.
...
# %% [markdown]
# Let us visualize the results. We show the predicted regularization parameter map, the ultra-low field adjoint
# reconstruction and the PDHG reconstruction with the learned regularization parameter map.
# %%
show_images(
