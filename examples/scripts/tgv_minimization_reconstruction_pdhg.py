...
# %%
from mrpro.algorithms.optimizers.tgv import make_tgv_op

dim = (-2, -1)  # The last two dimensions are the spatial dimensions in the image
x_shape = img_sense_24.shape
...
# %%
initial_x = img_sense_24.data
initial_v = torch.zeros(v_shape, dtype=torch.complex64)
initial_values = torch.cat((initial_x.unsqueeze(0), initial_v), dim=0)  # Concatenate initial x and v

print(f'Shape of initial_values: {initial_values.shape}')  # Should be torch.Size([3, 1, 1, 1, 256, 256]) in our example

(result_pdhg_24,) = mrpro.algorithms.optimizers.pdhg(
    f=f,
    g=g,
    operator=K,
...
# %%
from mrpro.operators.functionals.SSIM import ssim3d

img_direct_402.rss().min(), img_direct_402.rss().max()
data_range = img_direct_402.rss().max()

print(
    'SSIM of img_direct_24:',
    ssim3d(img_direct_402.rss().squeeze(0), img_direct_24.rss().squeeze(0), data_range=data_range),
)

print(
    'SSIM of img_sense_24:',
    ssim3d(img_direct_402.rss().squeeze(0), img_sense_24.rss().squeeze(0), data_range=data_range),
)

print(
    'SSIM of img_pdhg_24:', ssim3d(img_direct_402.rss().squeeze(0), img_pdhg_24.abs().squeeze(0), data_range=data_range)
)
