    ...
    def __init__(
        ...
        self.output_blocks = self.output_blocks[::-1]
         for n_head_current, n_head_next in pairwise(n_heads):
            self.down_blocks.append(Sequential(ConvND(dim)(n_head_current*n_channels_per_head, n_head_next*n_channels_per_head   // 2**dim, kernel_size=3, padding=1, bias=False), PixelUnshuffle(2)))
            self.up_blocks.append(Sequential(ConvND(dim)(n_head_next*n_channels_per_head, n_head_current*n_channels_per_head*2**dim, kernel_size=3, padding=1, bias=False), PixelShuffle(2)))

        self.middle_block = blocks(n_heads, n_blocks)
        self.last = Sequential(*blocks(n_heads[0],n_refinement_blocks), ConvND(dim)(n_channels_per_head*n_heads[0], channels_out, kernel_size=3, stride=1, padding=1))
