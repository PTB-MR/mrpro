...
class FastFourierOp(LinearOperator):
    ...
    def __init__(
        ...
        Applies a Fast Fourier Transformation along selection dimensions.

        If both recon_matrix and encoding_matrix are set, it will perform padding/cropping before and 
        ...
        If these are `SpatialDimension`s, the transform dimensions must be within the last three dimensions, corrospopnding to
        the typical (k2,k1,k0) and (z,y,x) axes of KData and IData.

        The transformation is done with 'ortho' normalization, i.e. the normalization constant is split between 
        forward and adjoint. See https://numpy.org/doc/stable/reference/routines.fft.html for an explantion.
        ...
        dim
            ...
            as these corrospond to k2, k1, and k0 of KData.
        encoding_matrix
            ...
            k-space shape in all 3 dimensions (k2, k1, k0), but only values in the dimensions specified in dim will be used.
        ...
        recon_matrix
            ...
            If encoding_matrix and recon_matrix are None, no padding or cropping will be performed.
            If all values in dim are -3, -2 or -1, this can also be a SpatialDimension describing the 
            image-space shape in all 3 dimensions (z,y,x), but only values in the dimensions specified in dim will be used.
            Otherwise, it should be a Sequence of the same length as dim.
        """
        super().__init__()
        self._dim = tuple(dim)
        ...
        if isinstance(recon_matrix, SpatialDimension):
            original_shape = [astuple(recon_matrix)[d] for d in dim]
            if not all(d in (-1,-2,-3) for d in dim):
                raise NotImplementedError(f'recon_matrix can only be a SpatialDimension if each value in dim is in (-3,-2,-1), got {dim=}'
        ...
        if isinstance(encoding_matrix, SpatialDimension):
            padded_shape = [astuple(encoding_matrix)[d] for d in dim]
            if not all(d in (-1,-2,-3) for d in dim):
                raise NotImplementedError(f'recon_matrix can only be a SpatialDimension if each value in dim is in (-3,-2,-1), got {dim=}'
                                          'Instead, you can also supply a list of values of same length as dim'
                )
        ...
        if encoding_matrix is not None and recon_matrix is not None:
            ...
            raise ValueError('Either encoding_matrix and recon_matrix must both be set to None or both to a value, got'
