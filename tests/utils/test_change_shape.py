"""Tests for functions to change the shape of data objects."""
...
from mrpro.utils.change_shape import sel_other_subset_from_kdata
from mrpro.utils.change_shape import split_idx
from mrpro.utils.change_shape import split_k1_into_other
...
@pytest.fixture(params=({'seed': 0, 'nother': 10, 'nk2': 6, 'nk1': 100},))
...
def test_split_k1_into_other(consistently_shaped_kdata, monkeypatch, nother_split, other_label):
    ...
    assert kdata_split.data.shape == (nother_split * nother, ncoils, nk2, ni_per_block, nk0)
    # Trajectory was the same for all "original" other dimensions so it is now only nother_split
    assert kdata_split.traj.broadcasted_shape == (nother_split, nk2, ni_per_block, nk0)
    # Verify new other label describes splitted data
    assert getattr(kdata_split.header.encoding_limits, other_label).length == nother_split
