...
def test_l1_functional_shape(
    ...
    torch.testing.assert_close(
        l1_norm.forward(x, dim=dim, keepdim=keepdim)[0].shape, shape_forward_x, rtol=1e-3, atol=1e-3
    )
...
def test_l1_prox_sigma_0(
    ...
    l1_norm = L1Norm(weight=1, divide_by_n=True)
    torch.testing.assert_close(l1_norm.prox(x, sigma=0.0)[0], x, rtol=1e-3, atol=1e-3)
...
def test_l1_prox_W_0(
    ...
    l1_norm = L1Norm(weight=0.0, divide_by_n=True)
    torch.testing.assert_close(l1_norm.prox(x, sigma=1)[0], x, rtol=1e-3, atol=1e-3)
    l1_norm = L1Norm(weight=0.0, divide_by_n=False)
    torch.testing.assert_close(l1_norm.prox(x, sigma=1)[0], x, rtol=1e-3, atol=1e-3)
...
def test_l1_moreau(
    ...
    sigma = 0.5
    x_new = l1_norm.prox(x, sigma=sigma)[0] + sigma * (l1_norm.prox_convex_conj(x / sigma, 1.0 / sigma))[0]
...
def test_l1_functional_componentwise(
    ...
    l1_norm = L1NormViewAsReal(weight=1.0, target=torch.tensor([0.5 + 0j, 1.5 + 0j, -0.5 + 0j]))
