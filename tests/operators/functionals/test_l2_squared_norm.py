...
def test_l2_functional_shape(
    ...
    l2_norm = L2NormSquared(weight=1)
    torch.testing.assert_close(l2_norm.forward(x,dim=dim,keepdim=keepdim)[0].shape,shape_forward_x , rtol=1e-3, atol=1e-3)
...
def test_l2_moreau(
    ...
    sigma = 0.5
    x_new = (l2_norm.prox(x, sigma=sigma)[0] + sigma * (l2_norm.prox_convex_conj(x / sigma, 1./sigma))[0])
