...
from mrpro.operators.Functional import SeparableSumFunctionals, SeparableSumProximableFunctionals
...
def test_separablesumfunctionals_shorthand():
    a = Dummy()
    b = ProxDummy()
    ...
    # if the types hints in SeparableSumFunctionals are not correct.
    mypy_test_dummy1: SeparableSumFunctionals[torch.Tensor, torch.Tensor] = summed  # noqa F841
    torch.testing.assert_close(summed(x1, x2)[0], a(x1)[0] + b(x2)[0])

    summed2left = b & (a & b)
    mypy_test_dummy2: SeparableSumFunctionals[torch.Tensor, torch.Tensor, torch.Tensor] = summed2left  # noqa F841
    torch.testing.assert_close(summed2left(x1, x2, x3)[0], b(x1)[0] + a(x2)[0] + b(x3)[0])

    summed2right = (a & b) & b
    mypy_test_dummy3: SeparableSumFunctionals[torch.Tensor, torch.Tensor, torch.Tensor] = summed2left  # noqa F841
    torch.testing.assert_close(summed2right(x1, x2, x3)[0], a(x1)[0] + b(x2)[0] + b(x3)[0])

    summed3 = (a & b) & (b & a)
    mypy_test_dummy4: SeparableSumFunctionals[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor] = summed3  # noqa F841
    torch.testing.assert_close(summed3(x1, x2, x3, x4)[0], a(x1)[0] + b(x2)[0] + b(x3)[0] + a(x4)[0])


def test_separablesumfunctionals_prox():
    rng = RandomGenerator(123)
    xs = rng.float32_tensor(4)
    summed = ProxDummy(1.0) & (ProxDummy(2.0) & ProxDummy(3.0)) & ProxDummy(4.0)
    ...
    # mypy will complain about the type of the expression and the argument not matching
    # if the types hints in SeparableSumProximableFunctionals are not correct.
    mypy_test_dummy1: SeparableSumProximableFunctionals[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor] = summed  # noqa F841
    mypy_test_dummy2: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor] = actual_prox  # noqa F841


def test_separablesumfunctionals_prox_convex_conj():
    rng = RandomGenerator(123)
    xs = rng.float32_tensor(4)
    summed = ProxDummy(1.0) & (ProxDummy(2.0) & ProxDummy(3.0)) & ProxDummy(4.0)
...
def test_separablesumfunctionals_iter_order():
    a = Dummy()
    b = ProxDummy()
    c = ProxDummy()
...
def test_separablesumfunctionls_len():
    a = Dummy()
    b = ProxDummy()
    assert len(a & b) == 2
    assert len(b & b & b) == 3
