"""Tests for FLASH signal model."""

from collections.abc import Sequence

import pytest
import torch
from mrpro.operators.models import FLASH
from mrpro.utils import RandomGenerator
from tests import autodiff_test
from tests.operators.models.conftest import SHAPE_VARIATIONS_SIGNAL_MODELS


def test_flash_special_values(parameter_shape: Sequence[int] = (2, 5, 10, 10, 10)) -> None:
    """Test FLASH signal at special input values."""
...
def test_flash_shape(
    ...
    model = FLASH(flip_angle=flip_angle, echo_time=echo_time, repetition_time=repetition_time)
    (signal,) = model(m0, t1, t2star)
    assert signal.shape == signal_shape
    assert signal.isfinite().all()


def test_autodiff_flash(parameter_shape: Sequence[int] = (2, 5, 10, 10)) -> None:
    """Test autodiff works for flash model."""
    model = FLASH(flip_angle=0.1, echo_time=1e-3, repetition_time=10e-3)
...
@pytest.mark.cuda
def test_flash_cuda(parameter_shape: Sequence[int] = (2, 5, 10, 10, 10)) -> None:
    """Test the flash model works on cuda devices."""
    rng = RandomGenerator(1)
    m0 = rng.complex64_tensor(parameter_shape, low=1e-10, high=10)
    t1 = rng.float32_tensor(parameter_shape, low=1e-10, high=2)
    t2star = rng.float32_tensor(parameter_shape, low=1e-10, high=0.5)
