...
def test_KNoise_to_complex128(random_test_data):
    ...
    noise = KNoise.from_tensor_and_kheader(data=random_test_data).to(dtype=torch.complex128)
...
def test_KNoise_cuda(random_test_data):
    ...
    noise = KNoise.from_tensor_and_kheader(data=random_test_data).cuda()
...
def test_KNoise_cpu(random_test_data):
    ...
    noise_cuda = KNoise.from_tensor_and_kheader(data=random_test_data).cuda()
    noise_cpu = noise_cuda.cpu()
    assert noise_cpu.data.is_cpu
    assert noise_cuda.data.is_cpu
