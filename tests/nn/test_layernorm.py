...
def test_layernorm_basic(
    ...
    if n_channels is not None:
        assert norm.weight is not None, (
            "Weight should not be None when n_channels is provided"
        )
        assert norm.bias is not None, (
            "Bias should not be None when n_channels is provided"
        )
...
def test_layernorm_invalid_cond_dim() -> None:
    ...
    with pytest.raises(
        RuntimeError, match="Trying to create tensor with negative dimension"
    ):
...
def test_layernorm_conditioning_features_last() -> None:
    ...
    assert output.shape == x.moveaxis(1, -1).shape, (
        f"Output shape {output.shape} != expected shape"
    )
...
def test_layernorm_gradient_flow() -> None:
    ...
    assert torch.isfinite(norm.weight.grad).all(), (
        "Weight gradients contain non-finite values"
    )
    assert torch.isfinite(norm.bias.grad).all(), (
        "Bias gradients contain non-finite values"
    )
