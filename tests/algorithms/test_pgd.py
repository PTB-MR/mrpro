...
from mrpro.operators import FastFourierOp, LinearOperatorMatrix, Operator, ProximableFunctionalSeparableSum
...
def test_pgd_on_functionals_with_multiple_inputs() -> None:
    ...
    l2 = L2NormSquared(target=kspace, divide_by_n=True)
    matrix_op = cast(Operator[torch.Tensor, torch.Tensor, tuple[torch.Tensor]], LinearOperatorMatrix(((fft, fft),)))
    f = l2 @ matrix_op
